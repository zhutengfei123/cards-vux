<template>
  <div class="classification-index">
    <tab bar-active-color="transparent" class="tab" ref="topBar">
      <tab-item :style="{'color':setColor}" :selected="index===0?true:false" v-for="(item, index) in tabsList" :key="index" @on-item-click="handleClickTabs(index)">
        <span class="title-bold">{{item.title}}</span>
        <span :class="{'active':index===isActive&&isShowBox}" class="app-icon">&#xe611;</span>
      </tab-item>
