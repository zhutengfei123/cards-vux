    ...
    <div class="order">
        ...
        <group v-for="(order,index) of orders" :key="order.order_sn" :style="{paddingTop:index===0&&'44px'}" @click.native="$router.push(`/order/detail/${order.order_sn}`)">
            ...
            <div v-for="(goods,index) of order.goods_list" :key="index">
                ...
                <p class="text gray center" @click="toggleShow(order)">查看全部{{goods.list.length}}件商品</p>
                <cell>
...
export default class Order extends Vue {
    ...
    onStatusChange (newValue, oldValue) {
      if (newValue !== oldValue) {
        ...
        this.getOrders({orderStatus: this.orderStatus}).then(msg => {
          ...
          this.orders.map(order => { !order.show && this.$set(order, 'show', false); });
        });
      }
    }

    get totalCount () {
      return goods => goods.list.reduce((sum, item) => (sum + Number(item.price)), 0);
    }
    ...
    created () {
      this.getOrders({orderStatus: this.orderStatus}).then(msg => { msg && this.$vux.toast.text(msg); });
    }
}
</script>
